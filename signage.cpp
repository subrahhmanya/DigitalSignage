		...
		if (m_bQuitting == false)
		{
					...
					|| ((wOK == false) && (wLastCheckM <= (time(0) - 300))))
			{
				if (wLastCheckM <= (time(0) - 300))
				{
					...
					sprintf(tWString, "http://free.worldweatheronline.com/feed/weather.ashx?q=%s,%s&format=xml&key=%s", tsWLoc.c_str(), tsWCountry.c_str(),
							sWAPI);
					if (debugLevel > 1)
			...
			if (wOK)
			{
				...
				if (cTime > (wUpdateTimer[1] + 5)) /* Temperature Alert Flash */
				{
					if ((wCelcius <= 3.0) || (wCelcius >= 25.0))
					{
						...
						{
							if (wCelcius <= 3.0)
								iBoxes[105].Create((char *) "Weather Temp Cold Alert Cycle", (char *) "", 0, weather[3].gltex(), 4, 0, 0,
										weather[3].width() / 2, weather[3].height() / 2, weather[3].width() / 2, weather[3].height() / 2, 255, 1, 1,
										(char *) "null", false, false, (char *) "", (char *) "", debugLevel);
							else
								iBoxes[105].Create((char *) "Weather Temp Hot Alert Cycle", (char *) "", 0, weather[2].gltex(), 4, 0, 0, weather[2].width() / 2,
										weather[2].height() / 2, weather[2].width() / 2, weather[2].height() / 2, 255, 1, 1, (char *) "null", false, false,
									...
									if ((atoi(ini.GetKeyValue("BoardSettings", "Enabled").c_str()) == 1) && (tBFound == false))
									{
										...
										for (int cB = 0; cB < 64; cB++)
										{
											if (strcmp("", mBoard[cB].UID) == 0)
											{
												...
												mBoard[cB].valR = atoi(ini.GetKeyValue("BoardSettings", "RedHue").c_str());
												mBoard[cB].valG = atoi(ini.GetKeyValue("BoardSettings", "GreenHue").c_str());
												mBoard[cB].valB = atoi(ini.GetKeyValue("BoardSettings", "BlueHue").c_str());
												...
												if (debugLevel > 1)
												{
													...
													for (int brdO = 0; brdO < mBoard[cB].nBoards; brdO++)
													{
														printf("\t\tChild Board %i (%i,%i)\n\t\t\tType\t\t%s\n\t\t\tSrc\t\t%s\n\t\t\tDuration\t%i\n", brdO + 1,
																cB, brdO, mBoard[cB].cBoard[brdO].Type, mBoard[cB].cBoard[brdO].Src,
																mBoard[cB].cBoard[brdO].Duration);
													}
						...
						if (mBoard[cB].CreatedID == 0)
						{
							...
							for (int cBC = 1; cBC < 65; cBC++) /* Intentionally do 1 to 65 */
							{
								...
								if (!iBoxes[cBC].isCreated())
								{
									...
									iBoxes[cBC].Create(mBoard[cB].UID, mBoard[cB].cBoard[mBoard[cB].curBoard].Src, mBoard[cB].TimeStampCFG, 0,
											mBoard[cB].Border, mBoard[cB].X, mBoard[cB].Y, mBoard[cB].Width, mBoard[cB].Height, sWidth, sHeight,
											mBoard[cB].Scale, tBoardType, mBoard[cB].curBoard, mBoard[cB].cBoard[mBoard[cB].curBoard].AudioOut,
											mBoard[cB].cBoard[mBoard[cB].curBoard].HeaderVis, mBoard[cB].HeaderVis,
											mBoard[cB].cBoard[mBoard[cB].curBoard].Header, mBoard[cB].Header, debugLevel);
...
void Signage::Draw()
{
	...
	if (!sInitDisp)
	{
		drawText("Starting DigitalSignage System...", fntCGothic[20], 3, 255, 255, 255, 255, 0, 1280, 350, 0, 0);
		sInitDisp = true;
	}
	else
	{
		...
		if (wOK)
		{
			...
			if (tWindSpeedMPH >= 25)
			{
				if (!iBoxes[104].isCreated() && iBoxes[104].stype() != -1)
				{
					iBoxes[104].Create((char *) "Weather Wind Alert", (char *) "", 0, weather[1].gltex(), 4, 0, 0, weather[1].width() / 2,
							weather[1].height() / 2, weather[1].width() / 2, weather[1].height() / 2, 255, 1, 1, (char *) "null", false, false, (char *) "",
							(char *) "", debugLevel);
				}
			}
			else
		...
		for (int n = 100; n < 128; n++)
		{
			if (iBoxes[n].isCreated())
			{
				...
				{
					/* Weather Box */
					if (!iBoxes[n].doDraw(4, -1, 255, 172, 172))
					{
						m_bQuitting = true;
						m_bRunning = false;
					}
